...
#if 1
static int __nvme_init_iod(struct request *rq, unsigned size,
		struct nvme_dev *dev, bool kv)
{
...
static int nvme_init_iod(struct request *rq, unsigned size,
		struct nvme_dev *dev)
{
	return __nvme_init_iod(rq, size, dev, false);
}
...
#if 1
static int nvme_kv_map_data(struct nvme_dev *dev, struct request *req,
		...
		unsigned size, struct nvme_command *cmnd)
{
	...
	if (is_kv_cmd(cmnd->common.opcode)) {
		return nvme_kv_map_data(dev, req, size, cmnd);
	}
#endif
	sg_init_table(iod->sg, req->nr_phys_segments);
	iod->nents = blk_rq_map_sg(q, req, iod->sg);
...
static enum blk_eh_timer_return nvme_timeout(struct request *req, bool reserved)
{
...
#ifdef REPORT_TIMEOUT
	{
		...
		if (req->cmd_type == REQ_TYPE_DRV_PRIV) {
			...
			if (req->rq_disk) {
				ns = req->rq_disk->private_data;
				if (ns) cmd.common.nsid = ns->ns_id;
			}
		}
		if (iod->kv_cmd)
		{
			__u32 *data = (__u32*) cmd.common.cdw2;
			...
			pr_err("nvme_timeout: non kv opcode (%02x) nsid(%d) request (%d).\n", cmd.common.opcode, cmd.common.nsid, req->errors);
		}
	}
#endif
#endif
