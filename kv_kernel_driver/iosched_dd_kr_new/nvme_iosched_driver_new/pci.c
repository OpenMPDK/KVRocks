...
#endif
		ret = kvs_nvme_map_data(dev, req, map_len, &cmnd);
			 ...
			 const struct blk_mq_queue_data *bd)
{
    ...
    if (ns && is_kv_dev(ns->ctrl)) {
        ...
        if (req->special) {
            ...
            cqe.command_id = cmnd.common.command_id;
            cqe.status = 0;
            if (is_kv_retrieve_cmd(cmnd.common.opcode)) {
                cqe.result = cmnd.common.cdw10[5];
            }
...
//#ifdef KV_NVME_SUPPORT
    ...
    if(qid == 1){
        printk("[%d :: %s]Set priority low for qid %d\n", __LINE__, __func__, qid);
        flags = NVME_QUEUE_PHYS_CONTIG | NVME_SQ_PRIO_LOW;
    }else{
        printk("[%d :: %s]Set priority urgent for qid %d\n", __LINE__, __func__, qid);
        flags = NVME_QUEUE_PHYS_CONTIG | NVME_SQ_PRIO_URGENT;
    }
...
static enum blk_eh_timer_return nvme_timeout(struct request *req, bool reserved)
{
...
#ifdef REPORT_TIMEOUT
    {
        ...
        cmd.common.nsid = 0;
        if (req->cmd_type == REQ_TYPE_DRV_PRIV) {
		    memcpy(&cmd, req->cmd, sizeof (struct nvme_command)); 
		    data = (__u32*) cmd.common.cdw2;
		    pr_err("nvme_timeout: opcode (%02x) nsid(%d) request (%d).\n", cmd.common.opcode, cmd.common.nsid, req->errors);
...
#ifdef KV_NVME_SUPPORT
    printk("[ %d : %s ] Real KV NVME IO Queue Count : %d(max : %d)\n", __LINE__, __func__, dev->queue_count, dev->max_qid);
#endif
...
#ifdef KV_NVME_SUPPORT
    printk("[ %d : %s ] KV NVME IO Queue Count : %d\n", __LINE__, __func__, nr_io_queues);
#endif
	if (result < 0)
		return result;
...
#ifdef KV_NVME_SUPPORT
    printk("[ %d : %s ] Max KV NVME IO Queue Count : %d\n", __LINE__, __func__, nr_io_queues);
#endif
