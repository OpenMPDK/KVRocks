...
//#ifdef KV_NVME_SUPPORT
#if 0
    printk("[%d :: %s]Set Weighted RR for Scheduling\n", __LINE__, __func__);
    ctrl->ctrl_config |= NVME_CC_ARB_WRRU | NVME_CC_SHN_NONE;
#else
...
static void nvme_alloc_ns(struct nvme_ctrl *ctrl, unsigned nsid)
{
     ...
     * CONFIG_NVM to support open SSD should be disabled.
     * So that, "else statement" can be conducted.
     */
...
uint64_t kv_proc_total_sync_wait_time;
...
static int kv_proc_show(struct seq_file *p, void *v) {
    ...
    //uint64_t local_kv_total_sync_wait_time = atomic64_read(&kv_total_sync_wait_time);
    ...
    //if(local_kv_total_sync_wait_time-kv_proc_total_sync_wait_time)
    //    seq_printf(p, "Total Sync Wait Time :     %llu : delta : %llumsec (per thread %lldusec)\n", local_kv_total_sync_wait_time/1000, (local_kv_total_sync_wait_time-kv_proc_total_sync_wait_time)/1000, (local_kv_total_sync_wait_time-kv_proc_total_sync_wait_time)/(local_kv_sync_del_req-kv_proc_sync_del_req + local_kv_sync_put_req-kv_proc_sync_put_req + local_kv_sync_get_req-kv_proc_sync_get_req));
    ...
    //kv_proc_total_sync_wait_time=                      local_kv_total_sync_wait_time;
...
static ssize_t kv_proc_write(struct file *filp,const char *buf,size_t count,loff_t *offp)
{
    ...
    atomic64_set(&kv_total_sync_wait_time, 0);
