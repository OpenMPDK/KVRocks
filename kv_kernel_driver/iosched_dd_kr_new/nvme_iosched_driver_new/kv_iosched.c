...
atomic64_t kv_total_sync_wait_time;
    ...
    *this_cpu_ptr(kv_iosched->percpu_memq[reserved_memq]) = new_memq;
    ...
    if(!list_empty(&mutable_memq->rq)){
...
#if 0
        {
            ...
            //queue_work_on(submitd->last_cpu, submitd->submit_wq, &submitd->submit_work);
            //queue_work_on(num_online_cpus() - 1, submitd->submit_wq, &submitd->submit_work);
            queue_work_on(0, submitd->submit_wq, &submitd->submit_work);
            ...
            if(submitd->last_cpu >= num_online_cpus())
                submitd->last_cpu = 0;
#endif
        }
#else
...
void kv_submission_workfn(struct work_struct *work)
{
    ...
    while(!list_empty(list)){
        ...
        ret = __nvme_submit_iosched_kv_cmd(memq, submitd->last_cpu);
        switch(submitd->type){
            case READ_DAEMON:
                submitd->last_cpu++;
                break;
...
restart:
    while((rq = kv_fifo_dequeue(&kv_iosched->complete_fifo))!=NULL)
    {
        ...
        if(!test_bit(KV_iosched_registered, &rq->iosched->state) && !atomic_read(rq->in_flight_rq))
            wake_up_atomic_t(rq->in_flight_rq);
...
int kv_iosched_init(struct kv_iosched_struct *kv_iosched, int node)
{
    ...
    kv_iosched->max_inflight_request = KV_MAX_INFLIGHT_REQUEST/num_online_cpus();
    for(i = 0 ; i < MAX_HOST_CPU ; i++)
        atomic_set(&kv_iosched->in_flight_rq[i], 0);
...
void kv_iosched_destroy(struct kv_iosched_struct *kv_iosched)
{
     ...
     */
    for(i = 0 ; i < MAX_HOST_CPU ; i++)
        wait_on_atomic_t(&kv_iosched->in_flight_rq[i], kv_wait_atomic_killable, TASK_KILLABLE);
