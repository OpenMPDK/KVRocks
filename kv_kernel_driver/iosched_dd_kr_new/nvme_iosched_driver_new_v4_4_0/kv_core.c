...
static int __nvme_submit_kv_user_cmd(struct kv_sync_request *sync_rq)
{
    ...
    if (key_addr && key_length > KVCMD_INLINE_KEY_MAX) {
        ...
        bip->bip_iter.bi_size = key_length;
        bip->bip_iter.bi_sector = 0;
...
static int nvme_submit_kv_multicmd(struct kv_sync_request *sync_rq_head, int count)
{
    ...
    for(sync_rq = sync_rq_head; sync_rq ; sync_rq = sync_rq->next) {
        ...
        if (sync_rq->key_addr && sync_rq->key_length > KVCMD_INLINE_KEY_MAX) {
            ...
            bip->bip_iter.bi_size = sync_rq->key_length;
            bip->bip_iter.bi_sector = 0;
...
static void bio_kern_endio(struct bio *bio)
{
    bio_put(bio);
}
...
int __nvme_submit_iosched_kv_cmd(struct kv_memqueue_struct *memq)
{
    ...
    list_for_each_safe(ele, next_ele, &memq->rq){ 
        ...
        /* Congestion control */
        if(KV_MAX_INFLIGHT_REQUEST  < atomic_inc_return(&rq->iosched->in_flight_rq)){
#ifndef NO_LOG
        ...
        struct nvme_passthru_kv_cmd __user *ucmd)
{
    ...
    if(key){
        ...
        if(kv_hash_find_read_and_invalidate(&kv_iosched->kv_hash[KV_RQ_HASH], key, cmd.key_length, hash_key)){
#ifndef NO_LOG
            atomic64_inc(&kv_total_discard_cancel_not_on_device); //not on device and cancel.
#endif
        }else{
#ifndef NO_LOG
            atomic64_inc(&kv_total_discard_on_device); //on device.
#endif
        }
...
#ifndef NO_LOG
            atomic64_inc(&kv_total_discard_ra); //in the readahead buffer and cacel.
#endif
        }else {
#ifndef NO_LOG
            atomic64_inc(&kv_total_discard_not_found_ra); //not readahead buffer.
#endif
        }
