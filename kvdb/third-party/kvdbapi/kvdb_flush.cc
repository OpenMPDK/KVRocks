...
//hainan.liu 
class DBFLUSHTest: public KVDBTest{
 public:
  DBFLUSHTest() {
    env_ = new EnvCounter(Env::Default());
    //env_ = new SpecialEnv(Env::Default());
    dbname_ = test::TmpDir(env_) + "/flush_test";
    //db_options_.create_if_missing = true;
    //db_options_.fail_if_options_file_error = true;
    //db_options_.env = env_;
    //DestroyDB(dbname_, Options(db_options_, column_family_options_));
    //EXPECT_OK(DestroyDB(dbname_, db_options_));
    cout << "---------------DBFLUSHTest--------------------"<< endl; 
...
*/
TEST_F(DBFLUSHTest, 1204_DeleteAndFlush) {
  Open();
...
TEST_F(DBFLUSHTest, 1133_ReadFromPersistedTier) {
//  do {
...
TEST_F(DBFLUSHTest, DISABLED_1119_SyncWALNotSupported){
   Open();
...
TEST_F(DBFLUSHTest, 1120_FlushWALNotSupported){
   Open();
...
TEST_F(DBFLUSHTest, 1121_NoFlushAndGet_Default) {
  Open();
  //Reopen();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
...
TEST_F(DBFLUSHTest, 1122_ReopenNoFlushAndGet_Default) {
  Open();
  Reopen();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
...
TEST_F(DBFLUSHTest, 1123_NoFlushAndGet_CF){
  Open();
  CreateColumnFamilies({"one"});
  // Generate log file A.
  //delete db_;
...
TEST_F(DBFLUSHTest, 1124_ReopenNoFlushAndGet_CF){
  Open();
  CreateColumnFamilies({"one"});
  // Generate log file A.
  //delete db_;
...
TEST_F(DBFLUSHTest, 1205_FlushEmptyDB) {
  Open();
  CreateColumnFamiliesAndReopen({"one", "two"});
  Reopen();
  Flush();
...
TEST_F(DBFLUSHTest, 1125_FlushAndGet) {
  Open();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
  ASSERT_OK(Put(0, "foo", "v1"));
...
TEST_F(DBFLUSHTest, 1126_FlushAndGet_CFs) {
  Open();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
...
TEST_F(DBFLUSHTest, 1127_FlushAndGet_NoWait) {
  Open();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
...
TEST_F(DBFLUSHTest, 1128_FlushAndGet_Wait) {
  Open();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  CreateColumnFamiliesAndReopen({"one", "two"});
  //delete db_;
...
TEST_F(DBFLUSHTest, 1129_FirstSnapshotTest) {
  Open();
  Options options = CurrentOptions();
...
TEST_F(DBFLUSHTest, 1130_MassPutTest) {
  Open();
  //CreateColumnFamiliesAndReopen({"one", "two"});
  //string k[16][1024]={0};
  //string v[16][1024]={0};
...
/*
TEST_F(DBFLUSHTest, 1131_FlushIterTest) {
  Open();
  CreateColumnFamiliesAndReopen({"one", "two"});
  ASSERT_OK(Put(0, "foo", "v1"));
  ASSERT_OK(Put(0, "bar", "v2"));
...
*/
TEST_F(DBFLUSHTest, 1132_FlushTest) {
  Open();
  CreateColumnFamiliesAndReopen({"one", "two"});
  ASSERT_OK(Put(0, "foo", "v1"));
  ASSERT_OK(Put(0, "bar", "v2"));
