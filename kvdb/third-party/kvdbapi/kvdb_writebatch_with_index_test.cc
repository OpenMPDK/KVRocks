...
class KVIter : public Iterator {
 public:
  ...
  virtual void SeekForPrev(const Slice& k) {
    ...
    iter_ = map_->lower_bound(k.ToString());  //lining.dou, upper_bound->lower_bound
    Prev();
  }
...
TEST_F(WriteBatchWithIndexTest, TestRandomIteraratorWithBase) {
  ...
  for (int rand_seed = 301; rand_seed < 366; rand_seed++) {
    ...
    for (auto key : source_strings) {
      ...
      int type = rnd.Uniform(5);  //lining.dou, 6->5
      switch (type) {
        case 0:
          // only base has it
          map[key] = value;
...
TEST_F(WriteBatchWithIndexTest, SingleDeleteTest) {
  ...
  value = PrintContents(&batch, nullptr);
  ASSERT_EQ("PUT(A):a,PUT(A):a2,DEL(A),PUT(B):b,", value);
  ...
  value = PrintContents(&batch, nullptr);
  ASSERT_EQ(
      "PUT(A):a,PUT(A):a2,DEL(A),PUT(A):a3,PUT(B):b,DEL(B),DEL(B)"
      ",PUT(C):c,DEL(C),",
      value);
  ...
  value = PrintContents(&batch, nullptr);
  ASSERT_EQ(
      "PUT(A):a,PUT(A):a2,DEL(A),PUT(A):a3,DEL(A),PUT(B):b,DEL(B),"
      "DEL(B),PUT(B):b4,PUT(C):c,DEL(C),PUT(C):c4,PUT(D):d4,"
      "DEL(D),DEL(D),",
      value);
}
