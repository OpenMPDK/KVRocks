...
TEST(CRC, Values_case) {
  ASSERT_NE(Value("a", 1), Value("A", 1));
}
...
TEST(CRC, Extend_Num) {
  ASSERT_EQ(Value("1234567890", 10),
            Extend(Value("12", 2), "34567890", 8));
}
TEST(CRC, Extend_Special) {
  ASSERT_EQ(Value("~!@#$%^&*()", 11),
            Extend(Value("~!@", 3), "#$%^&*()", 8));
}
...
TEST(CRC, Mask_Num) {
  uint32_t crc = Value("12345", 5);
  ASSERT_EQ(crc, Unmask(Mask(crc)));
}
TEST(CRC, Mask_Special) {
  uint32_t crc = Value("~!@#$%^&*()", 10);
  ASSERT_EQ(crc, Unmask(Mask(crc)));
}
}  // namespace crc32c
}  // namespace rocksdb
