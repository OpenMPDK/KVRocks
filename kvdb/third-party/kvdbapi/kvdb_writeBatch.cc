...
//}
//hainan.liu 
class WriteBatchTest: public KVDBTest {};

TEST_F(WriteBatchTest, 1105_Empty) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1030_Multiple) {
  WriteBatch batch;
  batch.Put(Slice("foo"), Slice("bar"));
  cout <<"0"<<endl;
  batch.Delete(Slice("box"));
  cout <<"1"<<endl;
//  batch.DeleteRange(Slice("bar"), Slice("foo"));  //Not supported
  cout <<"2"<<endl;
  batch.Put(Slice("baz"), Slice("boo"));
  cout <<"3"<<endl;
  //WriteBatchInternal::SetSequence(&batch, 100);
  //ASSERT_EQ(100U, WriteBatchInternal::Sequence(&batch));
  ASSERT_EQ(3, WriteBatchInternal::Count(&batch));  //should be 3, hainan.liu
//  ASSERT_EQ(
//      "Put(baz, boo)@103"
//      "Delete(box)@101"
//      "Put(foo, bar)@100"
...
TEST_F(WriteBatchTest, 1031_Append) {
  WriteBatch b1, b2;
  ...
  b2.Put("c", "cc");
  b2.Put("d", "dd");
  b2.MarkWalTerminationPoint(); //7月份测试;marks this point in the WriteBatch as the last record to be inserted into the WAL, provided the WAL is enabled
  b2.Put("e", "ee");
...
TEST_F(WriteBatchTest, 1032_SingleDeletion) {
  WriteBatch batch;
 // WriteBatchInternal::SetSequence(&batch, 100);
 // ASSERT_EQ("", PrintContents(&batch));
  ASSERT_EQ(0, batch.Count());
...
TEST_F(WriteBatchTest, 1033_PutNotImplemented) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1034_DeleteNotImplemented) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1035_SingleDeleteNotImplemented) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1036_MergeNotImplemented) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1037_Blob) {
  WriteBatch batch;
  batch.Put(Slice("k1"), Slice("v1"));
...
//之前2PC的write_batch是6月分设计的，后面美国把write_batch的marker全部改成定长的了，现在2PC这块的全部修改成定长的，要适配，工作量比较大，你们先把write_batch 那个preparecommit的用例注释掉把，2PC现在没有做，write_batch支持2PC也是没有意义的

/*
TEST_F(WriteBatchTest, 1038_PrepareCommit) {
  WriteBatch batch;
...
TEST_F(WriteBatchTest, 1039_Continue) {
  WriteBatch batch;
 ...
 // batch.PutLogData(Slice("blob2"));
  batch.Merge(Slice("foo"), Slice("bar"));
  batch.Merge(Slice("foo1"), Slice("bar1"));
  batch.Iterate(&handler);
  ASSERT_EQ(
      "Put(k1, v1)"
      "Put(k2, v2)"
...
TEST_F(WriteBatchTest, 1040_PutGatherSlices) {
  WriteBatch batch;
  batch.Put(Slice("foo"), Slice("bar"));
...
TEST_F(WriteBatchTest, 1041_ColumnFamiliesBatchTest) {
  WriteBatch batch;
  ...
  ASSERT_EQ(
      ...
      "DeleteCF(8, eightfoo)"
      "DeleteCF(2, twofoo)"
    //  "DeleteRangeCF(2, 3foo, 4foo)"  //not supported by kvdb
...
#ifndef ROCKSDB_LITE
TEST_F(WriteBatchTest, 1042_ColumnFamiliesBatchWithIndexTest) {
  ...
  ASSERT_EQ("eightfoo", iter->Entry().key.ToString());  //failed
...
TEST_F(WriteBatchTest, 1043_SavePointTest) {
  Status s;
  WriteBatch batch;
  batch.SetSavePoint();
...
TEST_F(WriteBatchTest, 1044_MemoryLimitTest) {
  Status s;
  // The header size is 12 bytes. The two Puts take 8 bytes which gives total
  // of 12 + 8 * 2 = 28 bytes.
  WriteBatch batch(0, 36);  //kvdb按照开发要求改成36
