...
struct DBOptions {
  ...
  //add num_cols which can be configured by user
  uint32_t num_cols = 0;
  ...
  // High watermark of key count for table cache
  uint64_t tablecache_high = 2000000;

  // max key size
  int max_key_size = 32;
  // max table size
  int max_table_size = 512*1024;
  // High watermark of cache size in bytes
  size_t max_cache_size = 2*1024*1024*1024UL;
  // insdb write worker count
  int num_write_worker = 8;
  // insdb max request size
  int max_request_size = 65536;
  // insdb max flush threshold
  size_t flush_threshold = 1024*1024;
  // insdb min flush update count
  uint32_t min_update_cnt_table_flush = 32*1024;
  // insdb write slowdown trigger
  uint32_t slowdown_trigger = 64*1024;
  // insdb align size
  uint32_t align_size = 4;
  // insdb prefetch cache size
  size_t max_uval_prefetched_size = 10*1024*1024;
  // insdb iterator cache size
  size_t max_uval_iter_buffered_size = 200*1024*1024;
  // insdb apporximate key size
  uint32_t approx_key_size = 16;
  // insdb apporximate value size
  uint32_t approx_val_size = 100;
  // insdb iterator prefetch hint
  uint32_t iter_prefetch_hint = 1;
  // insdb random get prefetch hint
  uint32_t random_prefetch_hint = 1;
  // insdb split prefix bits
  int split_prefix_bits = 0;
};
