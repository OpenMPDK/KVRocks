...
DEFINE_string(
    ...
    "reads\n"
    "\treadseqwhilewriting      -- 1 writer, N threads doing sequential "
    "reads\n"
...
DEFINE_int64(tablecache_low, 1000000,  // 1 million keys
             "Number of keys to use as a low watermark of table cache");

DEFINE_int64(tablecache_high, 2000000,  // 2 million keys
             "Number of keys to use as a high watermark of table cache");

DEFINE_int32(max_table_size, 512*1024,  // 512B
             "Max byte size of SKTable device format");

DEFINE_int64(max_cache_size, 250*1024*1024,  // 250MB
             "Byte size to use as max cache size including device formats of SKTables and key blocks");
...
DEFINE_bool(flush, false, "Flush all writes to disk");
...
DEFINE_int32(num_write_worker, 8, "number of workers");

DEFINE_int32(align_size, 4, "Device buffer alignment");

DEFINE_uint64(flush_threshold, 1024*1024, "ikey flush threshold");

DEFINE_int32(min_update_cnt_table_flush, 32*1024, "Minimum update count for table flush");

DEFINE_int32(slowdown_trigger, 64*1024, "PUT slowdown threshold");

DEFINE_uint64(max_uval_prefetched_size, 10*1024*1024, "prefetch size");

DEFINE_uint64(max_uval_iter_buffered_size, 4*1024*1024*1024UL, "iterator cache size");

DEFINE_int32(iter_prefetch_hint, 1, "iterator prefetch hint");

DEFINE_int32(random_prefetch_hint, 1, "random prefetch hint");

DEFINE_uint64(max_table_deviceformat, 1024*1024UL, "max count of table device formats");

DEFINE_int32(split_prefix_bits, 48, "table split prefix size");
...
class Benchmark {
 private:
  ...
  void PrintHeader() {
    ...
    fprintf(stdout, "max_table_size: %u\n", FLAGS_max_table_size);
    fprintf(stdout, "max_cache_size: %lu\n", FLAGS_max_cache_size);
    fprintf(stdout, "Max request size %u\n", FLAGS_max_req_size);
    fprintf(stdout, "num_write_worker %u\n", FLAGS_num_write_worker);
    fprintf(stdout, "align_size %u\n", FLAGS_align_size);
    fprintf(stdout, "flush_threshold %lu\n", FLAGS_flush_threshold);
    fprintf(stdout, "min_update_cnt_table_flush %u\n", FLAGS_min_update_cnt_table_flush);
    fprintf(stdout, "slowdown_trigger %u\n", FLAGS_slowdown_trigger);
    fprintf(stdout, "max_uval_prefetched_size %lu\n", FLAGS_max_uval_prefetched_size);
    fprintf(stdout, "max_uval_iter_buffered_size %lu\n", FLAGS_max_uval_iter_buffered_size);
    fprintf(stdout, "iter_prefetch_hint %u\n", FLAGS_iter_prefetch_hint);
    fprintf(stdout, "random_prefetch_hint %u\n", FLAGS_random_prefetch_hint);
    fprintf(stdout, "split_prefix_bits %d\n", FLAGS_split_prefix_bits);
 ...
 public:
  Benchmark()
    ...
    if (!FLAGS_use_existing_db) {
      ...
      options.max_table_size = FLAGS_max_table_size;
      options.max_cache_size = FLAGS_max_cache_size;
      options.max_request_size = FLAGS_max_req_size;
      options.num_write_worker = FLAGS_num_write_worker;
      options.align_size = FLAGS_align_size;
      options.flush_threshold = FLAGS_flush_threshold;
      options.min_update_cnt_table_flush = FLAGS_min_update_cnt_table_flush;
      options.slowdown_trigger = FLAGS_slowdown_trigger;
      options.max_uval_prefetched_size = FLAGS_max_uval_prefetched_size;
      options.max_uval_iter_buffered_size = FLAGS_max_uval_iter_buffered_size;
      options.iter_prefetch_hint = FLAGS_iter_prefetch_hint;
      options.random_prefetch_hint = FLAGS_random_prefetch_hint;
      options.split_prefix_bits = FLAGS_split_prefix_bits;
...
void VerifyDBFromDB(std::string& truth_db_name) {
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "readseqwhilewriting") {
        num_threads++;  // Add extra thread for writing
        method = &Benchmark::ReadSeqWhileWriting;
 ...
 private:
  ...
  Stats RunBenchmark(int n, Slice name,
    ...
    if (FLAGS_flush && db_.db)
        db_.db->Flush(FlushOptions(),db_.db->DefaultColumnFamily());
...
//    db->ReleaseSnapshot(options.snapshot);
  ...
  void ReadSeqWhileWriting(ThreadState* thread) {
    if (thread->tid > 0) {
  ...
  void BGWriter(ThreadState* thread, enum OperationType write_merge) {
    ...
    thread->stats.AddBytes(bytes);
    thread->stats.Stop();
    thread->stats.Report((write_merge == kWrite)?"bg randomwrite":"bg randommerge");
  }
