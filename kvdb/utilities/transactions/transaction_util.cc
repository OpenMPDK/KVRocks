...
Status TransactionUtil::CheckKeyForConflicts(DBImpl* db_impl,
  ...
  if (result.ok()) {
    SequenceNumber earliest_seq =
        db_impl->GetEarliestMemTableSequenceNumber(column_family, true);

    result = CheckKey(db_impl, column_family, earliest_seq, key_seq, key, cache_only);
#if 0
...
Status TransactionUtil::CheckKey(DBImpl* db_impl, ColumnFamilyHandle* column_family,
                                 SequenceNumber earliest_seq,
  ...
  if (result.ok()) {
    ...
    Status s = db_impl->GetLatestSequenceForKey(column_family, key, !need_to_read_sst,
...
#else
    ColumnFamilyHandle* column_family = db_impl->GetColumnFamilyHandle(cf_id);
...
#else
    db_impl->DestroyColumnFamilyHandle(column_family);
#endif
