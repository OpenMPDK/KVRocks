...
ImmutableDBOptions::ImmutableDBOptions(const DBOptions& options)
    : create_if_missing(options.create_if_missing),
      ...
      tablecache_low(options.tablecache_low),
      tablecache_high(options.tablecache_high),
      max_key_size(options.max_key_size),
      max_table_size(options.max_table_size),
      max_cache_size(options.max_cache_size),
      flush_threshold(options.flush_threshold),
      min_update_cnt_table_flush(options.min_update_cnt_table_flush),
      slowdown_trigger(options.slowdown_trigger),
      align_size(options.align_size),
      max_uval_prefetched_size(options.max_uval_prefetched_size),
      max_uval_iter_buffered_size(options.max_uval_iter_buffered_size),
      approx_key_size(options.approx_key_size),
      approx_val_size(options.approx_val_size),
      iter_prefetch_hint(options.iter_prefetch_hint),
      random_prefetch_hint(options.random_prefetch_hint),
      split_prefix_bits(options.split_prefix_bits)
{
}
...
void ImmutableDBOptions::Dump(Logger* log) const {
  ...
  ROCKS_LOG_HEADER(log, "            Options.tablecache_low: %lld", tablecache_low);
  ROCKS_LOG_HEADER(log, "            Options.tablecache_high: %lld", tablecache_high);
  ROCKS_LOG_HEADER(log, "            Options.tablecache_low_size: %lld", max_key_size);
  ROCKS_LOG_HEADER(log, "            Options.tablecache_low_size: %lld", max_table_size);
  ROCKS_LOG_HEADER(log, "            Options.tablecache_high_size: %lld", max_cache_size);
  ROCKS_LOG_HEADER(log, "            Options.flush_threshold: %lld", flush_threshold);
  ROCKS_LOG_HEADER(log, "            Options.min_update_cnt_table_flush: %d", min_update_cnt_table_flush);
  ROCKS_LOG_HEADER(log, "            Options.slowdown_trigger: %d", slowdown_trigger);
  ROCKS_LOG_HEADER(log, "            Options.align_size: %d", align_size); 
  ROCKS_LOG_HEADER(log, "            Options.max_uval_prefetched_size: %d", max_uval_prefetched_size);
  ROCKS_LOG_HEADER(log, "            Options.max_uval_iter_buffered_size: %d", max_uval_iter_buffered_size);
  ROCKS_LOG_HEADER(log, "            Options.approx_key_size: %d", approx_key_size);
  ROCKS_LOG_HEADER(log, "            Options.approx_val_size: %d", approx_val_size);
}
