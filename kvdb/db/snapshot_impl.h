...
#ifndef _ORIG_ROCKSDB_
  const insdb::Snapshot *insdb_snapshot_;
#endif
...
class SnapshotList {
 public:
  ...
  const SnapshotImpl* New(SnapshotImpl* s, SequenceNumber seq, uint64_t unix_time,
                    bool is_write_conflict_boundary) {
#else
  const SnapshotImpl* New(SnapshotImpl* s, const insdb::Snapshot *insdb_snapshot, uint64_t unix_time,
                        bool is_write_conflict_boundary) {
#endif
#ifdef _ORIG_ROCKSDB_
    s->number_ = seq;
#else
    s->number_ = insdb_snapshot->GetSequenceNumber();
    s->insdb_snapshot_ = insdb_snapshot;
#endif
