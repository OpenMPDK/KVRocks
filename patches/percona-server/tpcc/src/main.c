...
#define RTIME_DELIVERY 8*10000
#define RTIME_SLEV     2*10000
...
*/
  ...
  // CSV header line
  printf("CSV, time, neworder_trx, neworder_95percentile_rt, neworder_99percenttile_rt, neworder_maxrt, payment, payment_max_rt, order_status, order_status_max_rt, delivery, delivery_max_rt, stock_level, stock_level_max_rt\n"); 
  fflush(stdout);
  ...
  printf("\n<Raw Results>\n");
  for ( i=0; i<5; i++ ){
    printf("  [%d] success:%d late:%d  retry:%d  failure:%d avg_response_time: %.1f ms (response_time_limit: %d ms)\n",
           i, success[i], late[i], retry[i], failure[i],
           total_rt[i] / (success[i] + late[i]), rt_limit[i]);
  }
  printf(" in %d sec.\n", (measure_time / PRINT_INTERVAL) * PRINT_INTERVAL);

  printf("\n<Raw Results2(summerized version.)>\n");
  for( i=0; i<5; i++ ){
      success2_sum[i] = 0;
      late2_sum[i] = 0;
      retry2_sum[i] = 0;
  ...
  printf("        New-Order: %3.2f%% [OK, no minimum requirement]\n",f);
...
//  printf("%4d, %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f), %d:%.3f|%.3f(%.3f)\n",
  ...
  // must match the CSV header line
  printf("CSV, %llu, %d, %.3f, %.3f, %.3f, %d, %.3f, %d, %.3f, %d, %.3f, %d, %.3f\n",
	 time_count,
	 ( s[0] + l[0] - prev_s[0] - prev_l[0] ), percentile_val,percentile_val99,
	 (double)cur_max_rt[0],
	 ( s[1] + l[1] - prev_s[1] - prev_l[1] ),
